...
namespace amh {
  ...
  class disjAlgo : public algo<TIN,TOUT> {
  public:
    // chainAlgo(algo<TIN,TFOO>& _left, algo<TFOO,TOUT>& _right) :
    //   algo_left(_left), algo_right(_right) {}
    disjAlgo(algo<TIN,TOUT>& _left, algo<TIN,TFOO>& _right, bool b1=false, bool b2=false) :
      algo_left(_left), algo_right(_right), leak_left(b1), leak_right(b2) {}
    disjAlgo(disjAlgo<TIN,TFOO,TOUT>& _algo) :
    ...
    disjAlgo<TIN,TOUT,TFOO> disj(algo<TIN,TBAR>& _algo) {
      auto foo = new disjAlgo<TIN,TOUT,TFOO>(*this);
    ...
    template<class TBAR>
    disjAlgo<TIN,TOUT,TFOO> operator&&(algo<TIN,TBAR>& _algo) {
      return disj(_algo);
    }
